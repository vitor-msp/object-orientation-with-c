...
#define MAX_SIZE_STACK_TRACE 1000
int user_fd;
...
void analyze_stack(char *programName);
...
void print_stacktrace(int calledFromSigInt)
{
	...
	int stackLevel, getFunction = 0;
	char stackTrace[MAX_SIZE_STACK_TRACE], findedFunction[14];
	strcat(findedFunction, "setTotalValue");
	for (; i < (unsigned int)(nptrs - 2); ++i)
	{
		stackLevel = nptrs - 2 - i - 1;
		// if addr2line failed, print what we can
		if (addr2line(_programName, buffer[i], stackLevel) != 0)
		{
			if (getFunction == 0)
			{
				if (strstr(strings[i], findedFunction))
					getFunction = 1;
			}
			else if (getFunction == 1)
			{
				strcat(stackTrace, strings[i]);
				getFunction = 2;
			}
		}
	}
	write(user_fd, stackTrace, MAX_SIZE_STACK_TRACE);
